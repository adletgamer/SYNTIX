  ...
  return (
    <div className="flex flex-col items-center justify-center h-full min-h-[400px] space-y-8">
      ...
      <div
        ...
        className={`w-full max-w-md cursor-pointer border-2 border-dashed rounded-xl p-10 text-center transition-all duration-300 ${
          dragging ? 'border-biomarker/60 bg-biomarker/[0.06]' : 'border-white/10 bg-white/[0.02] hover:border-biomarker/30 hover:bg-biomarker/[0.03]'
        }`}
      >
        <input ref={inputRef} type="file" accept=".vcf,.vcf.gz" className="hidden" onChange={(e) => { if (e.target.files?.[0]) handle(e.target.files[0].name); }} />
        <p className="text-gray-400 text-sm">
          <span className="text-biomarker font-medium">Click to browse</span> or drag & drop
        </p>
        <p className="text-gray-600 text-xs mt-2">Accepts .vcf and .vcf.gz files</p>
      </div>
