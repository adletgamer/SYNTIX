...
import {
  ...
  FileCode2, Share2, LogOut, LayoutDashboard, Dna, FlaskConical,
  Wallet, ChevronRight
} from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
          ...
          <div key={i} className="group bg-white/[0.02] border border-white/[0.06] hover:border-white/[0.14] rounded-xl p-4 transition-all flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-6 text-sm">
              <div className="text-right">
                ...
                <p className="text-white font-medium">{a.royalty}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500">Earned</p>
                <p className="text-biomarker font-medium">{a.earnings}</p>
              </div>
            </div>
      ...
      <div className="bg-biomarker/[0.04] border border-biomarker/15 rounded-xl p-4 flex items-center gap-3">
        ...
        <p className="text-xs text-gray-400">All assets managed by SYNTIX HSM custody. Decryption keys never leave your device. GDPR Cryptographic Shredding available on request.</p>
      </div>
    </div>
  );
};
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="text-left">
          ...
          <p className="text-xs text-gray-500">Upload VCF → FHE encrypt → get ancestry, traits, PGx, risk scores & AI report</p>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-hero flex text-white font-sans">
      ...
      <aside className="w-60 border-r border-white/[0.05] bg-black/50 backdrop-blur-xl hidden md:flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 p-3 space-y-1">
          <p className="px-3 py-2 text-xs text-gray-600 uppercase tracking-widest">Guardian Node</p>
          ...
          <header className="mb-8 flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold tracking-tight">
                {active === 'overview'  && 'Biological Intelligence'}
                {active === 'analyser'  && 'Genome Analyser'}
                {active === 'vault'     && 'IP-NFT Vault'}
                {active === 'royalties' && 'Royalty Dashboard'}
              </h1>
              <p className="text-gray-500 text-sm mt-1">
                {active === 'overview'  && 'Guardian Node · FHE Encrypted Compute'}
                {active === 'analyser'  && 'VCF → FHE → Ancestry · Traits · PGx · PRS · AI Report'}
                {active === 'vault'     && 'Sovereign Biological IP on Story Protocol'}
                {active === 'royalties' && 'On-chain royalty payments from licensed datasets'}
              </p>
            </div>
